...
class Tableau
{
    ...
    public function generer_code(bool $export = false, bool $tri = false, bool $append = false): string
    {
        ...
        if (!$append) {
            $code = '<table class="' . CLASS_TABLE . '' . $this->class_tableau . '" id="{tab_id_mon_framework_html}">';
        } else {
            $code = '';
        }
        $code_3 = ''; // tableau CSV
        ...
        if ($this->header == '') {
            ...
            foreach ($this->colonnes as $colonne) {
                $class = $colonne['class'];
                $code .= '<th class="' . str_replace(' img-circle ', '', ' ' . $class . ' ') . '">' . ($colonne['libelle_entete'] != '' ? $colonne['libelle_entete'] : get_nom_colonne($colonne['DB_name'])) . ($tri ? "<a class=\"trier\" href=\"#\" onclick=\"trier_colonne_n('{tab_id_mon_framework_html}', $i)\"><span>&nbsp;</span></a>" : "") . "</th>"; //
                $i++;
                if ($export) {
            ...
            if ($tri) {
                ...
                foreach ($this->colonnes as $colonne) {
                    $class = $colonne['class'];
                    $code .= '<th class="' . str_replace(' img-circle ', '', ' ' . $class . ' ') . '"><a class="trier" href="#" onclick="' . "trier_colonne_n('{tab_id_mon_framework_html}', $i)" . '"><span>&nbsp;</span></a></th>'; //
                    $i++;
                    if ($export) {
                        $code_3 .= format_caractere_csv(get_nom_colonne($colonne['DB_name'])) . ';';
                    }
        ...
        if ($tri) { // COLONNE DE RECHERCHE
            $code .= '<tr id="ligne_recherche_{tab_id_mon_framework_html}" class="masquer">';
            $i = 0;
            foreach ($this->colonnes as $colonne) {
                $class = $colonne['class'];
                $code .= '<th class="' . str_replace(' img-circle ', '', ' ' . $class . ' ') . ' tableau_champ_recherche"><span><input type="text" id="{tab_id_mon_framework_html}_' . $i . '" oninput="recherche_tableau(\'{tab_id_mon_framework_html}\');"></span></th>'; //
                $i++;
            }
            $code .= '</tr>';
        }
        ...
        foreach ($this->donnees as $ligne_key => $ligne) {
            ...
            if (($num_page - 1) * $nb_elements_par_page <= $compteur_ligne && $compteur_ligne < $num_page * $nb_elements_par_page) {
                ...
                if (EDIT_TABLE_ROW != '') {
                    if ($lien != '') {
                        $code .= "<td class='text-right'><span style='display: inline-block; position: absolute; right:20px;'>$lien" . EDIT_TABLE_ROW . "$lien_fin</span></td>";
                    } else {
                        $code .= '<td></td>';
                    }
                }
        ...
        if ($compteur_ligne == 0) {
            $code = str_replace('{tab_id_mon_framework_html}', md5($code), $code); // Calcul de l'ID
        ...
        if (!$append) {
            ...
            if ($tri) {
                $retour .= " <a class=\"rechercher\" href=\"#tab_filtre_switch\" onclick=\"afficher_ligne_tri('{tab_id_mon_framework_html}')\"><span>&nbsp;</span></a>";
            }
            ...
            return str_replace('{tab_id_mon_framework_html}', md5($retour), $retour); // Calcul de l'ID
        } else {
            return str_replace('{tab_id_mon_framework_html}', md5($code), $code); // Calcul de l'ID
        }
    }
}
