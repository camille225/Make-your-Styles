...
class table_commande_monframework extends entite
{
    ...
    public static function initialiser_structure()
    {
        ...
        if (! test_si_table_existe(inst('commande'))) {
            executer_requete_mysql('CREATE TABLE '.inst('commande').'(Code_commande BIGINT UNSIGNED NOT NULL AUTO_INCREMENT, PRIMARY KEY (Code_commande)) ENGINE=MyISAM;', array_search('commande', LISTE_TABLES_HISTORIQUE_DESACTIVE) === false);
        }

        $liste_colonnes = lister_les_colonnes('commande');
        ...
        if (isset($liste_colonnes['Code_utilisateur'])) {
            ...
            executer_requete_mysql('ALTER TABLE '.inst('commande').' ADD Code_utilisateur BIGINT UNSIGNED NOT NULL;', array_search('commande', LISTE_TABLES_HISTORIQUE_DESACTIVE) === false);
        }
        $liste_colonnes_a_indexer['Code_utilisateur'] = 'Code_utilisateur';
        ...
        foreach ($liste_colonnes_a_indexer as $colonnes_a_indexer) {
            executer_requete_mysql('ALTER TABLE `' . inst('commande') . '` ADD INDEX(`' . $colonnes_a_indexer . '`);', array_search('commande', LISTE_TABLES_HISTORIQUE_DESACTIVE) === false);
        }
    }

    /**
     * Retourne la tructure de la table « commande » avec les valeurs initialisées par défaut.
     * @return array
     */
    public function mf_get_structure(): array
    {
        ...
        $struc = [
            ...
            'Code_utilisateur' => 0, // REF
        ];
        mf_formatage_db_type_php($struc);
        Hook_commande::pre_controller($struc['commande_Prix_total'], $struc['commande_Date_livraison'], $struc['commande_Date_creation'], $struc['Code_utilisateur'], $struc['Code_commande']);
        return $struc;
    }
    ...
    public function mf_ajouter(?float $commande_Prix_total, string $commande_Date_livraison, string $commande_Date_creation, int $Code_utilisateur, ?bool $force = false)
    {
        ...
        $code_erreur = 0;
        // Typage
        $commande_Prix_total = is_null($commande_Prix_total) || $commande_Prix_total === '' ? null : mf_significantDigit((float) str_replace(' ', '', str_replace(',', '.', $commande_Prix_total)), 6);
        ...
        elseif (! Hook_commande::autorisation_ajout($commande_Prix_total, $commande_Date_livraison, $commande_Date_creation, $Code_utilisateur) ) $code_erreur = REFUS_COMMANDE__AJOUT_BLOQUEE;
            ...
            $commande_Prix_total = is_null($commande_Prix_total) ? 'NULL' : (float) $commande_Prix_total;
            $commande_Date_livraison = format_date($commande_Date_livraison);
            $commande_Date_creation = format_date($commande_Date_creation);
        ...
        return ['code_erreur' => $code_erreur, 'Code_commande' => $Code_commande, 'callback' => ( $code_erreur==0 ? Hook_commande::callback_post($Code_commande) : null )];
    }
    ...
    public function mf_ajouter_2(array $ligne, bool $force = null) // array('colonne1' => 'valeur1',  [...] )
    {
        ...
        global $mf_initialisation;
        $Code_utilisateur = (isset($ligne['Code_utilisateur']) ? $ligne['Code_utilisateur'] : get_utilisateur_courant('Code_utilisateur'));
        $commande_Prix_total = (isset($ligne['commande_Prix_total'])?$ligne['commande_Prix_total']:$mf_initialisation['commande_Prix_total']);
        $commande_Date_livraison = (isset($ligne['commande_Date_livraison'])?$ligne['commande_Date_livraison']:$mf_initialisation['commande_Date_livraison']);
        $commande_Date_creation = (isset($ligne['commande_Date_creation'])?$ligne['commande_Date_creation']:$mf_initialisation['commande_Date_creation']);
        // Typage
        $Code_utilisateur = (int) $Code_utilisateur;
        $commande_Prix_total = is_null($commande_Prix_total) || $commande_Prix_total === '' ? null : mf_significantDigit((float) str_replace(' ', '', str_replace(',', '.', $commande_Prix_total)), 6);
        $commande_Date_livraison = format_date($commande_Date_livraison);
    ...
    public function mf_ajouter_3(array $lignes) // array( array( 'colonne1' => 'valeur1', 'colonne2' => 'valeur2',  [...] ), [...] )
    {
        ...
        foreach ($lignes as $ligne) {
            $Code_utilisateur = (int) (isset($ligne['Code_utilisateur']) ? intval($ligne['Code_utilisateur']) : 0);
            $commande_Prix_total = is_null((isset($ligne['commande_Prix_total']) ? $ligne['commande_Prix_total'] : $mf_initialisation['commande_Prix_total'])) ? 'NULL' : (float) (isset($ligne['commande_Prix_total']) ? $ligne['commande_Prix_total'] : $mf_initialisation['commande_Prix_total']);
    ...
    {
        $commande = $this->mf_get_2($Code_commande, ['autocompletion' => false]);
    ...
    public function mf_modifier( int $Code_commande, ?float $commande_Prix_total, string $commande_Date_livraison, string $commande_Date_creation, int $Code_utilisateur, ?bool $force = null)
    {
        if ($force === null) {
            $force = false;
        }
        ...
        $commande = $this->mf_get_2( $Code_commande, ['autocompletion' => false, 'masquer_mdp' => false]);
        ...
        elseif ( !Hook_commande::autorisation_modification($Code_commande, $commande_Prix_total, $commande_Date_livraison, $commande_Date_creation, $Code_utilisateur) ) $code_erreur = REFUS_COMMANDE__MODIFICATION_BLOQUEE;
            ...
            if (self::$lock[$Code_commande] == 0) {
                self::$cache_db->add_lock((string) $Code_commande);
            }
            ...
            self::$lock[$Code_commande]--;
            if (self::$lock[$Code_commande] == 0) {
                self::$cache_db->release_lock((string) $Code_commande);
        ...
        return ['code_erreur' => $code_erreur, 'callback' => ($code_erreur == 0 ? Hook_commande::callback_put($Code_commande) : null)];
    }
    ...
    public function mf_modifier_2(array $lignes, ?bool $force = null) // array( $Code_commande => array('colonne1' => 'valeur1',  [...] ) )
    {
        ...
        foreach ($lignes as $Code_commande => $colonnes) {
            if ($code_erreur == 0) {
                ...
                $Code_utilisateur = (isset($colonnes['Code_utilisateur']) && ( $force || mf_matrice_droits(['api_modifier_ref__commande__Code_utilisateur', 'commande__MODIFIER']) ) ? $colonnes['Code_utilisateur'] : (isset($commande['Code_utilisateur']) ? $commande['Code_utilisateur'] : 0 ));
                $commande_Prix_total = (isset($colonnes['commande_Prix_total']) && ( $force || mf_matrice_droits(['api_modifier__commande_Prix_total', 'commande__MODIFIER']) ) ? $colonnes['commande_Prix_total'] : ( isset($commande['commande_Prix_total']) ? $commande['commande_Prix_total'] : '' ) );
                $commande_Date_livraison = (isset($colonnes['commande_Date_livraison']) && ( $force || mf_matrice_droits(['api_modifier__commande_Date_livraison', 'commande__MODIFIER']) ) ? $colonnes['commande_Date_livraison'] : ( isset($commande['commande_Date_livraison']) ? $commande['commande_Date_livraison'] : '' ) );
                $commande_Date_creation = (isset($colonnes['commande_Date_creation']) && ( $force || mf_matrice_droits(['api_modifier__commande_Date_creation', 'commande__MODIFIER']) ) ? $colonnes['commande_Date_creation'] : ( isset($commande['commande_Date_creation']) ? $commande['commande_Date_creation'] : '' ) );
                // Typage
                $Code_utilisateur = (int) $Code_utilisateur;
                $commande_Prix_total = is_null($commande_Prix_total) || $commande_Prix_total === '' ? null : mf_significantDigit((float) str_replace(' ', '', str_replace(',', '.', $commande_Prix_total)), 6);
                $commande_Date_livraison = format_date($commande_Date_livraison);
    ...
    public function mf_modifier_4( int $Code_utilisateur, array $data, ?array $options = null /* $options = array( 'cond_mysql' => [], 'limit' => 0 ) */ ) // $data = array('colonne1' => 'valeur1', ... )
    {
        $code_erreur = 0;
        $Code_utilisateur = intval($Code_utilisateur);
        $mf_colonnes_a_modifier=[];
        if (isset($data['commande_Prix_total']) || array_key_exists('commande_Prix_total', $data)) { $mf_colonnes_a_modifier[] = 'commande_Prix_total = ' . format_sql('commande_Prix_total', $data['commande_Prix_total']); }
        if (isset($data['commande_Date_livraison']) || array_key_exists('commande_Date_livraison', $data)) { $mf_colonnes_a_modifier[] = 'commande_Date_livraison = ' . format_sql('commande_Date_livraison', $data['commande_Date_livraison']); }
        if (isset($data['commande_Date_creation']) || array_key_exists('commande_Date_creation', $data)) { $mf_colonnes_a_modifier[] = 'commande_Date_creation = ' . format_sql('commande_Date_creation', $data['commande_Date_creation']); }
        if (isset($data['Code_utilisateur'])) { $mf_colonnes_a_modifier[] = 'Code_utilisateur = ' . round($data['Code_utilisateur']); }
        if (count($mf_colonnes_a_modifier) > 0) {
            // cond_mysql
            $argument_cond = '';
    ...
    public function mf_supprimer(int $Code_commande, ?bool $force = null)
    {
        ...
        {
            $copie__commande = $this->mf_get($Code_commande, ['autocompletion' => false]);
            $this->supprimer_donnes_en_cascade("commande", [$Code_commande]);
    ...
    public function mf_supprimer_2(array $liste_Code_commande, ?bool $force = null)
    {
        ...
        $code_erreur = 0;
        $copie__liste_commande = $this->mf_lister_2($liste_Code_commande, ['autocompletion' => false]);
        $liste_Code_commande=[];
    ...
    public function mf_lister_contexte(?bool $contexte_parent = null, ?array $options = null /* $options = [ 'cond_mysql' => [], 'tris' => [], 'limit' => [], 'toutes_colonnes' => TOUTES_COLONNES_DEFAUT, 'liste_colonnes_a_selectionner' => [], 'autocompletion' => AUTOCOMPLETION_DEFAUT, 'autocompletion_recursive' => AUTOCOMPLETION_RECURSIVE_DEFAUT, 'controle_acces_donnees' => CONTROLE_ACCES_DONNEES_DEFAUT, 'maj' => true ] */)
    {
        ...
        if (! $contexte_parent && $mf_contexte['Code_commande'] != 0) {
            ...
            return [$commande['Code_commande'] => $commande];
        } else {
            return $this->mf_lister(isset($est_charge['utilisateur']) ? $mf_contexte['Code_utilisateur'] : 0, $options);
        }
    }

    public function mf_lister(?int $Code_utilisateur = null, ?array $options = null /* $options = [ 'cond_mysql' => [], 'tris' => [], 'limit' => [], 'toutes_colonnes' => TOUTES_COLONNES_DEFAUT, 'liste_colonnes_a_selectionner' => [], 'autocompletion' => AUTOCOMPLETION_DEFAUT, 'autocompletion_recursive' => AUTOCOMPLETION_RECURSIVE_DEFAUT, 'controle_acces_donnees' => CONTROLE_ACCES_DONNEES_DEFAUT, 'maj' => true ] */)
    {
        if ($options === null) {
            $options=[];
        }
            ...
            while ($nouvelle_lecture) {
                ...
                if (false === $retour = self::$cache_db->read($cle)) {
                    ...
                    if (! $nouvelle_lecture) {
                        ...
                        Hook_commande::mettre_a_jour([$row_requete['Code_commande'] => $row_requete]);
                    }
                }
            }
    ...
    public function mf_get_last(?int $Code_utilisateur = null, ?array $options = null /* $options = [ 'autocompletion' => AUTOCOMPLETION_DEFAUT, 'autocompletion_recursive' => AUTOCOMPLETION_RECURSIVE_DEFAUT, 'toutes_colonnes' => true, 'maj' => true ] */)
    {
        ...
        if (false === $retour = self::$cache_db->read($cle)) {
            ...
            if ($row_requete = mysqli_fetch_array($res_requete, MYSQLI_ASSOC)) {
                $Code_commande = intval($row_requete['Code_commande']);
            }
            mysqli_free_result($res_requete);
    ...
    public function mf_prec_et_suiv( int $Code_commande, ?int $Code_utilisateur = null, ?array $options = null /* $options = [ 'cond_mysql' => [], 'tris' => [], 'limit' => [], 'toutes_colonnes' => TOUTES_COLONNES_DEFAUT, 'liste_colonnes_a_selectionner' => [], 'autocompletion' => AUTOCOMPLETION_DEFAUT, 'autocompletion_recursive' => AUTOCOMPLETION_RECURSIVE_DEFAUT, 'controle_acces_donnees' => CONTROLE_ACCES_DONNEES_DEFAUT, 'maj' => true ] */)
    {
        if ($options === null) {
            $options=[];
        }
    ...
    public function mf_compter(?int $Code_utilisateur = null, ?array $options = null /* $options = [ 'cond_mysql' => [] ] */)
    {
        if ($options === null) {
            $options=[];
        }
    ...
    public function mfi_compter( array $interface, ?array $options = null /* $options = [ 'cond_mysql' => [] ] */ )
    {
        ...
        $Code_utilisateur = isset($interface['Code_utilisateur']) ? intval($interface['Code_utilisateur']) : 0;
        return $this->mf_compter( $Code_utilisateur, $options );
    }

    public function mf_liste_Code_commande(?int $Code_utilisateur = null, ?array $options = null /* $options = [ 'cond_mysql' => [] ] */)
    {
        if ($options === null) {
            $options=[];
        }
    ...
    public function mf_liste_Code_utilisateur_vers_liste_Code_commande( array $liste_Code_utilisateur, ?array $options = null /* $options = [ 'cond_mysql' => [] ] */ )
    {
        if ($options === null) {
            $options=[];
        }
    ...
    public function mf_liste_Code_commande_vers_liste_Code_utilisateur( array $liste_Code_commande, ?array $options = null /* $options = [ 'cond_mysql' => [] ] */ )
    {
        if ($options === null) {
            $options=[];
        }
    ...
    public function mf_search_commande_Prix_total(float $commande_Prix_total, ?int $Code_utilisateur = null): int
    {
        return $this->rechercher_commande_Prix_total($commande_Prix_total, $Code_utilisateur);
    }

    public function mf_search_commande_Date_livraison(string $commande_Date_livraison, ?int $Code_utilisateur = null): int
    {
        return $this->rechercher_commande_Date_livraison($commande_Date_livraison, $Code_utilisateur);
    }

    public function mf_search_commande_Date_creation(string $commande_Date_creation, ?int $Code_utilisateur = null): int
    {
        return $this->rechercher_commande_Date_creation($commande_Date_creation, $Code_utilisateur);
    }

    /**
     * Trouve le premier "Code_commande" rattaché à "Code_utilisateur"
     * Si pas de résultat, 0 sera retourné
     * @param int $Code_utilisateur
     * @return int $Code_commande
     */
    public function mf_search_Code_utilisateur(int $Code_utilisateur): int
    {
        return $this->rechercher_commande__Code_utilisateur($Code_utilisateur);
    }

    public function mf_search__colonne(string $colonne_db, $recherche, ?int $Code_utilisateur = null): int
    {
        switch ($colonne_db) {
    ...
    {
        $res_requete = executer_requete_mysql('SELECT AUTO_INCREMENT as next_id FROM INFORMATION_SCHEMA.TABLES WHERE table_name = \'' . inst('commande') . '\';', false);
        $row_requete = mysqli_fetch_array($res_requete, MYSQLI_ASSOC);
        mysqli_free_result($res_requete);
    ...
    public function mf_search(array $ligne): int // array('colonne1' => 'valeur1',  [...] )
    {
        ...
        if ($row_requete = mysqli_fetch_array($res_requete, MYSQLI_ASSOC)) {
            $r = intval($row_requete['Code_commande']);
        } else {
            $r = 0;
        }
        mysqli_free_result($res_requete);
