...
class Mf_Cachedb
{
    ...
    {
        $this->dossier_cache = get_dossier_data('db', 'cache');
    ...
    public function write(string $cle, $variable, bool $ram = true)
    {
        if (!MODE_PROD) {
    ...
    {
        if (file_exists($this->microtime)) {
            $plancher = filemtime($this->microtime) - DELAI_EXECUTION_WORKER;
            $liste = glob("{$this->dossier_cache}*");
            foreach ($liste as $filename) {
                if (file_exists($filename)) {
                    if (filemtime($filename) < $plancher) {
                        @unlink($filename);
                    }
                }
