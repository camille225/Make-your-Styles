...
class Mf_Cachedb
{
    ...
    {
        if (! MODE_PROD) {
            if (strlen(serialize($variable)) > 8 * 1024 * 1024) {
                ...
                echo '<html><head><title>Bah, qu\'est ce qui se passe ?</title></head><body style="background-color: black; color: #ffd400; padding: 10px; font-family: monospace; font-size: 16px;"><h1>Requête volumineuse</h1>' . str_replace('  ', '&nbsp; ', str_replace('  ', '&nbsp; ', str_replace(PHP_EOL, '<br>', $txt))) . '</body></html>';
                exit();
            }
        }
        global $mf_cache_volatil;
    ...
    function release_lock(string $id)
    {
