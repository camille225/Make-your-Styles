...
*/
    ...
    $cle = md5("get_a_filtrer_$Code_utilisateur, $Code_vue_utilisateur");
    $signature = (string) $cache->read($cle);
    while (! $nouvelle_lecture) {
        $mf_cache_volatil = new Mf_cache_volatil(); // suppression du cache volatil
        $retour_json['get'] = $db->a_filtrer()->mf_get($Code_utilisateur, $Code_vue_utilisateur, ['autocompletion' => true]);
