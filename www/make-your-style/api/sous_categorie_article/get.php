...
*/
    ...
    $cle = md5("get_sous_categorie_article_$Code_sous_categorie_article");
    $signature = (string) $cache->read($cle);
    while (! $nouvelle_lecture) {
        $mf_cache_volatil = new Mf_cache_volatil(); // suppression du cache volatil
        $retour_json['get'] = $db->sous_categorie_article()->mf_get( $Code_sous_categorie_article, ['autocompletion' => true]);
